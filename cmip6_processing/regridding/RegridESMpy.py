...
def build_regridder_2d(src_rep, dst_rep, regrid_method, mask_threshold):
    ...
    regridding_arguments = {
        ...
        #'extrap_method':ESMF.ExtrapMethod.NEAREST_STOD, #TH 02-11-20, fills in all masked values, and then could re-apply same mask for all, other options exist like creepfill
        #http://www.earthsystemmodeling.org/esmf_releases/public/ESMF_8_0_0/ESMF_refdoc/node5.html#SECTION050121100000000000000
    
    if np.ma.is_masked(src_rep.data): #if source has mask (this determines values or zeros apparently, diff tas vs zos?)
        src_field.data[...] = ~src_rep.data.mask.T #apply mask to data, and extract mask
        ...
        mask_regridder = ESMF.Regrid(
            ...
            **regridding_arguments) #this regrids the mask
        regr_field = mask_regridder(src_field, dst_field)
        
        #anywhere where the interpolation is lower than threshold (default .99) apply mask
        #masked grids are presumably 0 here. All ocean values would give a value of 1. If anything lower than 1, we take it as land directly 
    ...
    def regridder(src):
        ...
        regr_field = field_regridder(src_field, dst_field) #regrid stuff
        res.data[...] = regr_field.data[...].T
        res.mask[...] = dst_mask #destination mask applied here, I think derived from source mask somehow
        return res
...
def build_regridder(src_rep, dst_rep, method, mask_threshold=.99):
    ...
    if src_rep.ndim == 2: # or <3 for AWI?
